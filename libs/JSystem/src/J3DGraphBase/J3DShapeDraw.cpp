...
bool triangulate_draw(std::vector<u8>& out, u8 opcode, const u8* vertices, u32 stride, u16 vtxCount) {
    switch (opcode) {
    case GX_TRIANGLES:
...
void append_triangles_to_run(std::vector<u8>& out, MergeRun& run, u8 cmd, const std::vector<u8>& vertices,
                                    u32 stride) {
    u32 offset = 0;
...
#else
        if (opcode != GX_TRIANGLEFAN && opcode != GX_TRIANGLESTRIP)
            break;
#endif
        int vtxNum = be16(*((u16*)(dlStart + offset + 1)));
        count += vtxNum;
...
#if TARGET_PC
J3DShapeDraw::J3DShapeDraw(const u8* displayList, u32 displayListSize, const GXVtxDescList* vtxDesc) {
    u32 stride = 0;
    std::vector<u8> optimized;
    if (calc_vtx_stride(vtxDesc, stride) && optimize_display_list(displayList, displayListSize, stride, optimized)) {
