...
A `.dusk` carries one top-level native library per platform/architecture, named exactly `<platform>-<arch><ext>`: `windows-amd64.dll`, `windows-arm64.dll`, `linux-x86_64.so`, `linux-aarch64.so`, `darwin-arm64.dylib`, `darwin-x86_64.dylib`, `android-aarch64.so`, `android-x86_64.so`. The loader looks up its own platform-arch name — nothing else is considered (libraries under subdirectories like `res/` are treated as assets). `add_dusk_mod` produces the right name automatically; shipping multiple platforms is just adding the other builds' libraries to the same zip. If the bundle contains top-level libraries but not the host's, the mod is disabled with an error rather than loading a mismatched library. A bundle with no libraries at all is loaded as an asset-only mod. Universal (multi-arch) macOS binaries are not supported — build per-arch. Mods can be enabled, disabled, and reloaded at runtime from the Mods window; the enabled setting persists (see [Runtime Lifecycle](#runtime-lifecycle)).

**Android.** Code mods are supported on Android: build with an `android-*` preset and `add_dusk_mod` produces an `android-aarch64.so` (mods link `libmain.so` directly — the game is a shared library there, so the game ABI surface is its dynamic symbol table). The mods directory is `mods/` inside the app's data directory (visible under Android/data via the Dusklight documents provider, or push with `adb`). Native libraries are always extracted to internal app storage before loading, since external storage is mounted non-executable. By-name symbol resolution (`HookService::resolve`) is currently unavailable on Android (no symbol manifest ships in the APK).
