...
// Session-lifetime (never recreated), so worker callbacks may read it from the global.
...
bool build_composite_pipeline(
    ...
    // Multiply blend: fragment output is the visibility multiplier (result = dst * src).
...
bool ensure_targets(uint32_t width, uint32_t height) {
    ...
    bool ok = createStorageTexture(
                  "AO preprocessed depth", WGPUTextureFormat_R32Float, 5, g_targets.preprocessedDepth) &&
        createStorageTexture("AO noisy", WGPUTextureFormat_R32Float, 1, g_targets.aoNoisy) &&
        createStorageTexture(
            "AO depth differences", WGPUTextureFormat_R32Uint, 1, g_targets.depthDifferences) &&
    ...
    if (ok) {
        g_targets.preprocessedDepthAll = wgpuTextureCreateView(g_targets.preprocessedDepth, nullptr);
...
// Render worker thread: the whole AO chain as one compute pass with three dispatches.
void on_compute(
    ...
    WGPUBindGroup mip4Group = makeBindGroup(g_mip4Layout,
        {textureEntry(6, data.preprocessedDepthMips[3]),
    ...
    WGPUBindGroup gtaoGroup = makeBindGroup(g_gtaoLayout,
        {textureEntry(0, data.preprocessedDepthAll), textureEntry(1, g_hilbertLutView),
            textureEntry(2, data.aoNoisy), textureEntry(3, data.depthDifferences),
            uniformEntry(4)});
    WGPUBindGroup denoiseGroup = makeBindGroup(g_denoiseLayout,
        {textureEntry(0, data.aoNoisy), textureEntry(1, data.depthDifferences),
    ...
    wgpuComputePassEncoderDispatchWorkgroups(pass,
        div_ceil(std::max(data.width >> 4, 1u), 8), div_ceil(std::max(data.height >> 4, 1u), 8),
        1);
...
void on_draw(
    ...
    WGPUBindGroupLayout layout =
        data.debug_view != 0 ? g_compositeDebugLayout : g_compositeLayout;
...
void on_scene_after_opaque(ModContext*, const GfxStageContext*, void*) {
    ...
    std::memcpy(uniforms.inverse_projection, camera.view_from_proj,
        sizeof(uniforms.inverse_projection));
...
ModResult build_controls_tab(
    ...
    control.help_rml = "AO shows raw visibility as grayscale. Normals shows the view-space "
                       "normals the GTAO pass consumes. Depth shows the preprocessed depth "
                       "as a distance gradient. Staircase detects quantized depth: smooth "
...
MOD_EXPORT ModResult mod_initialize(ModError* error) {
    ...
    if (!build_compute_pipeline("AO preprocess depth", g_preprocessSource, "preprocess_depth",
        ...
        !build_compute_pipeline("AO denoise", g_denoiseSource, "spatial_denoise",
