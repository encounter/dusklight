...
// Drawn into the EFB pass with multiply blending (srcFactor = Dst, dstFactor = Zero), so the
// fragment output is the multiplier; 1.0 leaves the scene untouched.
//
...
//       depth reads near-black with thin bright triangle edges; bright speckle/stripes
//       across surface interiors mean the depth values are quantized into steps. The ratio
//       is self-normalizing — it cannot be hidden by tone-mapping choices.
...
// Manual bilinear sample (r32float is unfilterable without optional device features); this is
// what makes half-resolution AO upscale smoothly.
...
// ---- Debug-view helpers: mirrors of the GTAO pass's input pipeline ----
...
// Raw-snapshot variant of load_depth for the staircase view (WGSL cannot pass textures to
// functions, so the helper is duplicated for the second texture).
...
fn fs_main(in: VertexOutput) -> @location(0) vec4f {
    if uniforms.debug_view == 2u {
        // Reconstructed view-space normals, [-1,1] -> RGB. Sky (depth 0) will be garbage by
        // construction; look for corruption on real geometry.
    ...
    if uniforms.debug_view == 4u {
        // Staircase detector on the raw snapshot depth (see header comment).
