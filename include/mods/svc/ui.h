 ...
 * dialogs, scoped RCSS style sheets, and document-stack queries.
 *
 * All calls must be made on the game thread, from mod callbacks (initialize,
 * update, hooks, or UI callbacks). Handles are opaque, generation-checked ids;
 * a stale or foreign handle fails with MOD_INVALID_ARGUMENT instead of
 * dereferencing freed UI state. Element handles die with the content that owns
 * them: a panel or tab rebuild destroys the previous build's elements, so
 * re-acquire handles inside the build callback rather than caching them across
 * builds. Strings are UTF-8 and, in both directions, only valid for the
...
typedef enum UiStyleScope {
    ...
    UI_SCOPE_GRAPHICS_TUNER = 5, /* the graphics tuner sub-window */
} UiStyleScope;
...
typedef enum UiControlBinding {
    /* Values flow through the `get`/`set` callbacks. Getters are polled every
     * frame while the control is visible and must be cheap. */
    UI_BINDING_CALLBACKS = 0,
    /* The control reads and writes `config_var` (a ConfigService handle owned
     * by the calling mod) directly: persistence, change notifications and the
     * modified indicator (value != default) are wired automatically. The var
     * type must match the control kind: TOGGLE = bool, NUMBER and SELECT =
     * int, STRING = string. Float vars are not bindable; use callbacks. */
    UI_BINDING_CONFIG_VAR = 1,
} UiControlBinding;

/* Tagged by the control's kind: TOGGLE reads bool_value, NUMBER and SELECT
 * read int_value, STRING reads string_value. string_value passed to a setter
 * is only valid during the call; a getter should point it at storage owned by
 * the mod (e.g. a static buffer) that stays valid until the next call into the
 * mod — the host copies it right after the getter returns. */
...
typedef struct UiControlDesc {
    ...
    const char* label;
    /* Optional RML shown as contextual help when the control is focused or
     * hovered. Only rendered where a help pane exists (mod window tabs);
     * ignored in Mods-window panels. */
    const char* help_rml;
    ...
    /* Optional override for the modified indicator. CONFIG_VAR controls derive
     * it from value != default when this is NULL. */
    UiPredicateFn is_modified;
    /* Passed to every callback above. */
    void* user_data;
    /* NUMBER: inclusive clamp range and step. min == max means the defaults
     * (0 .. INT32_MAX); step < 1 means 1. */
    int64_t min;
    int64_t max;
    int64_t step;
...
typedef ModResult (*UiTabBuildFn)(ModContext* ctx, UiWindowHandle window,
    UiElementHandle left_pane, UiElementHandle right_pane, void* user_data, ModError* out_error);
...
typedef struct UiWindowDesc {
    ...
    const char* rcss;
    /* Fired when the window is destroyed for any reason (user close,
     * window_close, mod teardown, shutdown). The handle is already invalid. */
...
typedef struct UiService {
    ...
    /* --- minor 0: Mods-window panels, elements, controls --- */

    /* Register a panel shown in the calling mod's Mods-window tab. */
    ...
    ModResult (*pane_add_badge_row)(ModContext* ctx, UiElementHandle pane, const char* label,
        bool ok, UiElementHandle* out_elem);
     ...
     * rows, badge on badge rows, progress on progress bars). */
    ...
    ModResult (*elem_set_badge)(ModContext* ctx, UiElementHandle elem, bool ok);
    ModResult (*elem_set_progress)(ModContext* ctx, UiElementHandle elem, float value);

    /* --- minor 1: windows, dialogs, document stack --- */
    ...
    /* Whether any focus-stack document is currently visible (game input is
     * blocked while one is). */
    ModResult (*is_any_document_visible)(ModContext* ctx, bool* out_visible);
    /* Show and focus the top document of the stack. */
    ModResult (*focus_top_document)(ModContext* ctx);
    /* Ask the top document to close. Window-like documents (host and mod
     * windows, dialogs) comply; permanent documents (menu bar, pre-launch)
     * refuse with MOD_UNSUPPORTED. Cooperative: this can close host windows
     * the user has open. */
    ModResult (*close_top_document)(ModContext* ctx);

    /* --- minor 2: scoped styles --- */

    /* Register an RCSS sheet applied to every document of `scope`, now and in
     * the future, until unregistered or the calling mod is torn down. Sheets
     * apply in registration order after the host styles. Fails with
     * MOD_INVALID_ARGUMENT if the RCSS does not parse. */
    ModResult (*register_styles)(
        ModContext* ctx, UiStyleScope scope, const char* rcss, UiStyleHandle* out_style);
    /* Like register_styles, but reads the RCSS from the mod bundle's res/
     * directory (same path rules as ResourceService::load). MOD_UNAVAILABLE if
