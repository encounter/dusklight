 ...
 *
 * Registrations are owned by the calling mod and removed automatically (subscriptions
 * included) when it is disabled, reloaded, or fails; persisted values survive and are restored
 * by the next registration of the same name. Values are saved to config.json; writes are
 * debounced, not flushed per call.
 */
typedef struct ConfigService {
    ...
    ModResult (*get_string)(ModContext* ctx, ConfigVarHandle var, char* buffer,
