 ...
 * (hook_add_pre/hook_add_post/hook_set_replace over a &Class::method): they generate the
...
/* How set_replace resolves a second replace-hook on a target that already has one. */
...
typedef struct HookService {
     ...
     * same target; they run in priority then registration order. set_replace installs a single
