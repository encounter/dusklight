 ...
 * read stays in memory until it re-reads the file (usually on scene reload).
 *
 * disc_path names the file to overlay: absolute with a leading '/', matched against the disc
 * case-insensitively (e.g. "/res/Stage/F_SP103/R00_00.arc"). Paths that do not exist on the disc
 * are added as new files. If multiple sources overlay the same path, the last one wins: a mod's
 * runtime registrations beat its static overlay/ files, and later-loaded mods beat earlier ones.
 */
typedef struct OverlayService {
     ...
     */
    ModResult (*add_file)(ModContext* ctx, const char* disc_path, const char* bundle_path,
