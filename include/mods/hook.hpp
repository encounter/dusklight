...
inline size_t vcall_slot_offset(const void*& fn) noexcept {
    ...
    fn = p;
    if (p[0] != 0x48 || p[1] != 0x8B || p[2] != 0x01) {  // mov rax, [rcx]
        return npos;
    }
    if (p[3] == 0xFF && p[4] == 0x20) {  // jmp [rax]
        return 0;
    }
    if (p[3] == 0xFF && p[4] == 0x60) {  // jmp [rax + imm8]
        return static_cast<int8_t>(p[5]);
    }
    if (p[3] == 0xFF && p[4] == 0xA0) {  // jmp [rax + imm32]
        int32_t off;
        std::memcpy(&off, p + 5, 4);
        return off;
    }
    return npos;
...
ModResult hook_set_replace(
    ...
    return hooks->set_replace(mod_ctx, Entry::target, callback, options);
}

template <auto Target>
ModResult hook_set_replace(
    const HookService* hooks, HookReplaceFn callback, const HookOptions* options = nullptr) {
    return hook_set_replace<Hook<Target> >(hooks, callback, options);
}
