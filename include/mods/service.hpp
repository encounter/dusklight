...
public:
    ...
    [[nodiscard]] const Service* get() const { return m_slot != nullptr ? *m_slot : nullptr; }

    [[nodiscard]] const Service* const* slot() const { return m_slot; }
...
#define IMPORT_SERVICE_EX(                                                                         \
    ...
    const int mod_import_registration_##variable =                                                 \
    ...
    static const ::dusk::mods::ServiceRef<service_type> variable {                                 \
        &mod_import_slot_##variable                                                                \
    }
...
#define SERVICE_HAS_FIELD(service_ref, field)                                                      \
    SERVICE_HAS((service_ref).get(),                                                               \
        std::remove_cv_t<std::remove_pointer_t<decltype((service_ref).get())> >, field)
