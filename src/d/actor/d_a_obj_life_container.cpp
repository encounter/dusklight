...
int daObjLife_c::create() {
    ...
    if (!mIsPrmsInit) {
        ...
        if (mItemGiveVanillaNo == dItemNo_NONE_e) {
            mVanillaItemNo = parameterItemNo;
            const u8 resolvedItem =
                dusk::mods::item_check_freestanding(getSaveBitNo(), mVanillaItemNo, this);
            mItemGiveTag = dusk::mods::item_give_tag_freestanding(getSaveBitNo());
            mItemOverridden = resolvedItem != mVanillaItemNo;
            if (mItemOverridden) {
                fopAcM_SetParam(this, (params & 0xFFFFFF00) | resolvedItem);
            }
        } else {
            mVanillaItemNo = mItemGiveVanillaNo;
            mItemOverridden = parameterItemNo != mItemGiveVanillaNo;
        }
        mOverrideHover =
            mItemOverridden &&
            (mVanillaItemNo == dItemNo_UTAWA_HEART_e ||
                (mVanillaItemNo >= dItemNo_M_BEETLE_e && mVanillaItemNo <= dItemNo_F_MAYFLY_e));
#endif
        mIsPrmsInit = true;
    }
...
int daObjLife_c::initActionOrderGetDemo() {
    ...
    if (mItemOverridden) {
        m_itemNo = dusk::mods::item_check_tagged(mItemGiveTag, mVanillaItemNo, this);
    }
#endif
