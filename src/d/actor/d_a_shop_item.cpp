...
#if TARGET_PC
    if (m_itemNo != dItemNo_NONE_e && mItemGiveVanillaNo == dItemNo_NONE_e) {
        mItemGiveVanillaNo = m_itemNo;
        const u8 resolvedItem = dusk::mods::item_check_shop(mItemGiveVanillaNo, this);
        mItemOverridden = resolvedItem != mItemGiveVanillaNo;
        if (mItemOverridden) {
