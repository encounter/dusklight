...
fopAcM_prm_class* fopAcM_CreateAppend() {
    ...
    if (append != NULL) {
        ...
        append->argument = -1;
        IF_DUSK(append->mItemGiveVanillaNo = 0xFF;)
    }
    return append;
}
...
fopAcM_prm_class* createAppend(u16 i_setId, u32 i_parameters, const cXyz* i_pos, int i_roomNo,
    ...
    fpc_ProcID i_parentId IF_DUSK_ARG(u32 i_itemGiveTag) IF_DUSK_ARG(u8 i_itemVanillaNo)) {
    ...
    IF_DUSK(append->mItemGiveVanillaNo = i_itemVanillaNo;)
...
fopAc_ac_c* fopAcM_fastCreate(s16 i_procName, u32 i_parameters, const cXyz* i_pos, int i_roomNo,
    ...
    void* i_createFuncData IF_DUSK_ARG(u32 i_itemGiveTag) IF_DUSK_ARG(u8 i_itemVanillaNo)) {
    fopAcM_prm_class* append =
        createAppend(0xFFFF, i_parameters, i_pos, i_roomNo, i_angle, i_scale, i_argument,
            fpcM_ERROR_PROCESS_ID_e IF_DUSK_ARG(i_itemGiveTag) IF_DUSK_ARG(i_itemVanillaNo));
    if (append == NULL) {
        return NULL;
    }
...
fpc_ProcID fopAcM_createItemForBoss(const cXyz* i_pos, int i_itemNo, int i_roomNo,
    ...
    int _ = -1;
#if TARGET_PC
    const u8 vanillaItem = i_itemNo & 0xFF;
    u32 giveTag;
    if (i_itemCheckName != NULL) {
        i_itemNo = dusk::mods::item_check(i_itemCheckName, vanillaItem, NULL);
        giveTag = dusk::mods::item_give_tag(i_itemCheckName);
    } else {
        i_itemNo = dusk::mods::item_check_boss(vanillaItem, NULL);
    ...
    fopAc_ac_c* actor = fopAcM_fastCreate(fpcNm_Obj_LifeContainer_e, params, i_pos, i_roomNo,
        i_angle, i_scale, -1, NULL, NULL IF_DUSK_ARG(giveTag) IF_DUSK_ARG(vanillaItem));
    if (actor != NULL) {
