...
Component* build_mod_control(Pane& pane, Pane* helpPane, ModControlSpec spec) {
    // The spec is shared between the control's lambdas and the help-pane
    // builder, which re-runs on every focus
...
ModWindow::ModWindow(Desc desc) : mDesc(std::move(desc)) {
    // Lets mod RCSS scope selectors to mod windows without touching host ones
    mRoot->SetClass("mod-window", true);
...
bool ModWindow::consume_close_request() {
    // Owns the close so the document below is only re-shown when it was
    // visible when this window was pushed (a dialog opened during gameplay
    // must not pop the hidden menu bar open on dismiss)
    mDoAud_seStartMenu(kSoundWindowClose);
    pop(mDesc.showTopOnClose);
    return true;
}
