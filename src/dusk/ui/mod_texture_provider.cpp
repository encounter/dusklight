...
std::optional<CachedImage> load_mod_image(std::string_view idAndPath, std::string_view source) {
    ...
    // Runs on the main thread (RmlUi render), same thread as ModLoader::tick, so iterating
    // mods() is safe; the shared_ptr copy keeps the bundle alive across the read regardless.
...
std::optional<aurora::rmlui::RuntimeTexture> mod_texture_provider(std::string_view source) {
    ...
    // Keyed by the full source including ?rev=: a reload produces a new key and a fresh
    // decode; stale entries age out via the cache cap.
