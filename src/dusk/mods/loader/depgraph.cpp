...
#include "loader.hpp"
#include "native_module.hpp"  // complete NativeModule for ~LoadedMod when reordering m_mods
...
// Edges are built from ModManifestInfo rather than live manifests or the service registry:
// it survives dlclose and is populated regardless of enabled/failed state, so the graph can be
// rebuilt mid-session (a reload that changes a mod's imports/exports re-runs the sort).
...
bool in_required_cycle(
    ...
    std::vector<bool> visited(placed.size(), false);
    while (!pending.empty()) {
...
void sort_mods_by_dependencies(std::vector<std::unique_ptr<LoadedMod>>& mods) {
