...
bool Source::ready() const {
    return produced.load(std::memory_order_acquire) != consumed.load(std::memory_order_relaxed) ||
           finished.load();
}
...
bool Source::read_block(void* destination) {
    ...
    *header = {};
    ...
    std::memcpy(header + 1, blocks[index % blocks.size()].pcm.data(),
        channels * JASAramStream::getBlockSize());
