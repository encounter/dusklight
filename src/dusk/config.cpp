...
void Register(ConfigVarBase& configVar) {
    ...
    if (overridePair != UnregisteredConfigVarOverrides.end()) {
        // Deliberately kept in the map: launch-arg overrides are session-wide, so a CVar that is
        // unregistered and registered again later (e.g. across a mod reload) re-acquires it.
        try {
...
void unregister(ConfigVarBase& configVar) {
    ...
    // Stash exactly what Save() would have written so the value round-trips through config.json
    // and a later Register() of the same name restores it via the back-fill above.
