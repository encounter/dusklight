...
// Nonempty [a-z0-9_] segments separated by single periods.
bool is_valid_mod_id(std::string_view id);
// 1-31 bytes from [A-Za-z0-9._-], excluding "." and "..".
bool is_valid_save_name(std::string_view name);
// 1-64 bytes from [A-Za-z0-9_-].
bool is_valid_config_name(std::string_view name);

// A single relative path component without separators, colons, NULs, ".", or "..".
bool is_safe_path_component(std::string_view name);
// A relative resource path with no empty, ".", or ".." components.
