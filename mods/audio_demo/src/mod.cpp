...
struct View { UiElementHandle file{}, format{}, error{}, time{}, progress{}, next{}; };
std::unique_ptr<Track> track;
AudioStreamHandle stream{};
AudioStreamState snapshot{};
UiWindowHandle window{};
View panelView{}, windowView{};
struct Entry { uint64_t key{}; std::string location, name; };
struct LibraryView {
...
enum class Control { Choose, Play, Pause, Stop, Restart, Seek, Position, Volume, Pitch, Fade, Loop, Duck, Next, AddFile, AddFolder, ClearQueue, ClearHistory, Library };
void check(ModResult result, const char* operation) {
    if (result != MOD_OK) throw std::runtime_error{std::string{operation} + " failed (" + std::to_string(result) + ")."};
}
void report(const std::exception& error) {
    message = error.what();
    svc_log->error(mod_ctx, message.c_str());
}
uint32_t fade_frames() { return static_cast<uint32_t>(fadeMs * 32); }
double duration() { return track ? double(track->decoder.length_frames()) / track->decoder.sample_rate() : 0; }
void close_stream() {
    if (stream) svc_audio->close(mod_ctx, stream);
    stream = 0;
    snapshot = {};
...
void start_at(double seconds, bool shouldPlay) {
    if (!track) return;
    close_stream();
    const auto frame = std::min<uint64_t>(static_cast<uint64_t>(std::max(0.0, seconds) * track->decoder.sample_rate()),
...
void load_track(const Entry& entry) {
    check(svc_file->check(mod_ctx, entry.location.c_str()), "Accessing music (select it again if permission was revoked)");
...
void remember_track() {
    if (!track || track->remembered) return;
    ...
    if (history.size() > historyLimit) history.pop_back();
...
bool supported_name(const std::string& name) {
    ...
    return extension == "mp3" || extension == "ogg" || extension == "oga" || extension == "flac" || extension == "wav";
}
struct FolderEntries { std::vector<Entry> entries; bool limited{}; };
void listed(ModContext*, const FileEntry* entry, void* data) {
    if (!entry || entry->is_directory || !entry->name || !entry->location || !supported_name(entry->name)) return;
    auto& folder = *static_cast<FolderEntries*>(data);
    if (folder.entries.size() == queueLimit) { folder.limited = true; return; }
...
void picked(ModContext*, ModResult result, const char* const* locations, uint32_t count, const char* error, void* data) {
    picking = false;
    if (result == MOD_UNAVAILABLE) return;
    try {
        ...
        if (mode == Pick::Open) {
            ...
            if (mode == Pick::Folder) {
                ...
                std::sort(folder.entries.begin(), folder.entries.end(), [](const Entry& a, const Entry& b) {
                    return a.name < b.name;
                });
                limited = folder.limited;
                for (auto& entry : folder.entries) {
                    if (enqueue(std::move(entry))) ++added;
                    else limited = true;
                }
            } else {
                for (uint32_t i = 0; i < count; ++i) {
                    if (enqueue({0, locations[i], display_name(locations[i])})) ++added;
                    else limited = true;
                }
            }
            libraryMessage.clear();
            if (limited) libraryMessage = "Queue limit: 256 tracks.";
            else if (mode == Pick::Folder && !added) libraryMessage = "No supported audio files in this folder.";
            ++libraryRevision;
        }
    } catch (const std::exception& exception) {
        report(exception);
...
void choose(Pick mode) {
    ...
    check(mode == Pick::Folder ? svc_file->pick_folder(mod_ctx, &options, picked, data) :
        svc_file->pick_file(mod_ctx, &options, picked, data), "Opening file picker");
    picking = true;
}
...
void pressed(ModContext*, void* data) {
    try {
        ...
        case Control::Choose: choose(Pick::Open); break;
        case Control::AddFile: choose(Pick::Enqueue); break;
        case Control::AddFolder: choose(Pick::Folder); break;
        case Control::Next: play_queued(0); break;
        case Control::ClearQueue:
            queue.clear(); advancePending = false; libraryMessage.clear(); ++libraryRevision; break;
        case Control::ClearHistory:
            history.clear(); libraryMessage.clear(); ++libraryRevision; break;
        case Control::Library: open_window(mod_ctx, nullptr); break;
        case Control::Play:
            if (!track && !queue.empty()) { play_queued(0); break; }
            if (paused && stream) {
                ...
                playing = true;
                message.clear();
            } else if (startPending) { playing = true; paused = false; }
            else start_at(0, true);
            break;
        case Control::Pause:
            if (stream) check(svc_audio->pause(mod_ctx, stream, fade_frames()), "Pausing music");
            paused = true;
            playing = false;
            message.clear();
            break;
        case Control::Stop: stop(); break;
        case Control::Restart: start_at(0, true); break;
        case Control::Seek: start_at(duration() * seekPercent / 100.0, playing); break;
        default: break;
        }
    } catch (const std::exception& exception) { report(exception); }
}
bool disabled(ModContext*, void* data) {
    switch (static_cast<Control>(reinterpret_cast<uintptr_t>(data))) {
    case Control::Choose: return picking;
    case Control::AddFile: case Control::AddFolder: return picking || queue.size() >= queueLimit;
    case Control::Next: case Control::ClearQueue: return queue.empty();
    case Control::ClearHistory: return history.empty();
    case Control::Play: return (!track && queue.empty()) || playing;
    case Control::Pause: return !playing;
    case Control::Stop: return !stream && !startPending && !advancePending;
    case Control::Restart: case Control::Seek: case Control::Position: return !track;
    default: return false;
    }
}
void get_value(ModContext*, void* data, UiControlValue* value) {
    switch (static_cast<Control>(reinterpret_cast<uintptr_t>(data))) {
    case Control::Volume: value->int_value = volume; break;
    case Control::Pitch: value->int_value = pitch; break;
    case Control::Fade: value->int_value = fadeMs; break;
    case Control::Position: value->int_value = seekPercent; break;
    case Control::Loop: value->bool_value = loop; break;
    case Control::Duck: value->bool_value = duckBgm; break;
    default: break;
    }
}
void set_value(ModContext*, void* data, const UiControlValue* value) {
    try {
        ...
        case Control::Volume:
            ...
            if (stream) check(svc_audio->set_volume(mod_ctx, stream, volume / 100.0f, fade_frames()), "Setting volume");
            break;
        case Control::Pitch:
            pitch = std::clamp<int64_t>(value->int_value, 50, 200);
            if (stream) check(svc_audio->set_pitch(mod_ctx, stream, pitch / 100.0f), "Setting pitch");
            break;
        case Control::Fade: fadeMs = std::clamp<int64_t>(value->int_value, 0, 5000); break;
        case Control::Position: seekPercent = std::clamp<int64_t>(value->int_value, 0, 100); break;
        case Control::Loop: loop = value->bool_value; break;
        case Control::Duck:
            duckBgm = value->bool_value;
            if (stream || startPending) start_at(positionSeconds, playing);
            break;
        default: break;
        }
    } catch (const std::exception& exception) { report(exception); }
...
void pump() {
    ...
    if (advancePending) { advancePending = false; play_queued(0); return; }
    if (startPending) {
        ...
        if (result == MOD_UNAVAILABLE && ++openRetries < 300) return;
        check(result, "Opening audio stream (start the game and free another music stream if necessary)");
        check(svc_audio->set_pitch(mod_ctx, stream, pitch / 100.0f), "Setting pitch");
        if (playing) check(svc_audio->play(mod_ctx, stream, fade_frames()), "Starting playback");
        startPending = false;
        message.clear();
    }
    if (!stream) return;
    check(svc_audio->get_state(mod_ctx, stream, &snapshot), "Reading playback state");
    if (playing && (snapshot.phase == AUDIO_STREAM_PLAYING || snapshot.phase == AUDIO_STREAM_ENDED)) remember_track();
    positionSeconds = startSeconds + snapshot.position_frames / 32000.0;
    if (wrapped) positionSeconds = std::fmod(positionSeconds, duration());
    else positionSeconds = std::min(positionSeconds, duration());
    if (snapshot.phase == AUDIO_STREAM_ENDED) {
        if (loop && playing) start_at(0, true);
    ...
    for (unsigned batch = 0; batch < 4 && !eof; ++batch) {
        ...
        if (pendingOffset == pendingFrames) {
            ...
            if (!pendingFrames && loop) {
                if (!track->decoder.seek(0)) throw std::runtime_error{"Could not rewind the loop."};
                wrapped = true;
                pendingFrames = track->decoder.read({pcm.data(), count * track->decoder.channels()});
                if (!pendingFrames) throw std::runtime_error{"No audio could be decoded at the loop start."};
            }
            if (!pendingFrames) {
        ...
        uint32_t accepted{};
        check(svc_audio->write(mod_ctx, stream, pcm.data() + pendingOffset * track->decoder.channels(),
            std::min(budget, pendingFrames - pendingOffset), &accepted), "Writing audio");
...
ModResult update_view(ModContext*, void* data, ModError*) {
    ...
    char info[180]{};
    if (track) std::snprintf(info, sizeof(info), "%s · %u Hz · %s · %.1f MiB", track->decoder.format_name(),
        track->decoder.sample_rate(), track->decoder.channels() == 1 ? "Mono" : "Stereo", track->bytes.size / 1048576.0);
    else std::snprintf(info, sizeof(info), "MP3 · Ogg Vorbis · FLAC · WAV");
    ...
    set(view.next, queue.empty() ? "Queue empty" : "Up next: " + queue.front().name + " · " + std::to_string(queue.size()) + " queued");
    return svc_ui->elem_set_progress(mod_ctx, view.progress, duration() ? std::clamp(positionSeconds / duration(), 0.0, 1.0) : 0);
}
ModResult add_control(UiElementHandle pane, Control id, const char* label, UiControlKind kind = UI_CONTROL_BUTTON,
    int64_t min = 0, int64_t max = 0, int64_t step = 1, const char* suffix = nullptr) {
    UiControlDesc desc = UI_CONTROL_DESC_INIT;
    desc.kind = kind;
    desc.label = label;
    switch (id) {
    case Control::Restart: desc.icon = "replay"; break;
    case Control::Play: desc.icon = "play_arrow"; break;
    case Control::Pause: desc.icon = "pause"; break;
    case Control::Stop: desc.icon = "stop"; break;
    case Control::Next: desc.icon = "skip_next"; break;
    case Control::AddFile: desc.icon = "note_add"; break;
    case Control::AddFolder: desc.icon = "create_new_folder"; break;
    case Control::ClearQueue: case Control::ClearHistory: desc.icon = "delete"; break;
    default: break;
    }
    if (desc.icon != nullptr) desc.kind = UI_CONTROL_ICON_BUTTON;
    ...
    switch (id) {
    case Control::Play: case Control::Pause: case Control::Stop: case Control::Restart: case Control::Next:
        style = "music-transport"; break;
    case Control::Position: case Control::Seek: style = "music-seek"; break;
    case Control::Choose: style = "music-choose"; break;
    case Control::AddFile: style = "music-add-file"; break;
    case Control::AddFolder: style = "music-add-folder"; break;
    case Control::ClearQueue: case Control::ClearHistory: style = "music-clear"; break;
    case Control::Library: style = "music-library-action"; break;
    default: break;
    }
    return svc_ui->elem_set_class(mod_ctx, element, style, true);
}
UiElementHandle add_row(UiElementHandle parent, const char* style = nullptr,
    UiRowAlign align = UI_ROW_ALIGN_START) {
    UiRowDesc desc = UI_ROW_DESC_INIT;
    desc.align = align;
    UiElementHandle row{};
    check(svc_ui->pane_add_row(mod_ctx, parent, &desc, &row), "Building row");
    if (style) check(svc_ui->elem_set_class(mod_ctx, row, style, true), "Styling row");
    return row;
}
ModResult build_player(UiElementHandle pane, UiElementHandle settings, void* data) {
    try {
        ...
        check(svc_ui->elem_set_class(mod_ctx, settings, "music-settings", true), "Styling settings");
        const auto text = [&](UiElementHandle* element, const char* style) {
            ...
            check(svc_ui->elem_set_class(mod_ctx, *element, style, true), "Styling track information");
        };
        ...
        check(add_control(seek, Control::Position, "Position", UI_CONTROL_NUMBER, 0, 100, 5, "%"), "Building seek control");
        ...
        check(add_control(actions, Control::AddFolder, "Queue folder..."), "Building queue controls");
        if (pane == settings) check(add_control(pane, Control::Library, "Open music window..."), "Building library control");
        check(svc_ui->pane_add_section(mod_ctx, settings, "Playback"), "Building settings");
        check(add_control(settings, Control::Volume, "Volume", UI_CONTROL_NUMBER, 0, 200, 5, "%"), "Building volume control");
        check(add_control(settings, Control::Pitch, "Speed / pitch", UI_CONTROL_NUMBER, 50, 200, 5, "%"), "Building pitch control");
        check(add_control(settings, Control::Fade, "Fade time", UI_CONTROL_NUMBER, 0, 5000, 100, " ms"), "Building fade control");
        check(add_control(settings, Control::Loop, "Loop track", UI_CONTROL_TOGGLE), "Building loop control");
        check(add_control(settings, Control::Duck, "Duck game music", UI_CONTROL_TOGGLE), "Building ducking control");
        return update_view(mod_ctx, data, nullptr);
    } catch (const std::exception& exception) { report(exception); return MOD_ERROR; }
}
ModResult build_view(ModContext*, UiElementHandle pane, void* data, ModError*) {
    return build_player(pane, pane, data);
}
ModResult build_window(ModContext*, UiWindowHandle, UiElementHandle left, UiElementHandle right, void* data, ModError*) {
    return build_player(left, right, data);
}
void list_pressed(ModContext*, UiListHandle, uint64_t key, void* data) {
    try {
        if (data) {
            const auto found = std::find_if(history.begin(), history.end(), [key](const Entry& entry) { return entry.key == key; });
            if (found == history.end()) return;
            libraryMessage = enqueue(*found) ? "" : "Queue limit: 256 tracks.";
            ++libraryRevision;
        } else {
            const auto found = std::find_if(queue.begin(), queue.end(), [key](const Entry& entry) { return entry.key == key; });
            if (found != queue.end()) play_queued(static_cast<size_t>(found - queue.begin()));
        }
    } catch (const std::exception& exception) { report(exception); }
}
ModResult update_library(ModContext*, void*, ModError*) {
    auto& view = libraryView;
    if (view.revision == libraryRevision) return MOD_OK;
    try {
        const auto populate = [](UiListHandle list, const std::vector<Entry>& entries) {
            ...
            check(svc_ui->list_set_items(mod_ctx, list, items.data(), items.size()), "Updating music list");
        };
        populate(view.queue, queue);
        populate(view.history, history);
        check(svc_ui->elem_set_text(mod_ctx, view.queueSummary, (std::to_string(queue.size()) + " queued").c_str()), "Updating queue");
        check(svc_ui->elem_set_text(mod_ctx, view.historySummary, (std::to_string(history.size()) + " tracks").c_str()), "Updating history");
        check(svc_ui->elem_set_text(mod_ctx, view.notice, libraryMessage.c_str()), "Updating queue status");
        check(svc_ui->elem_set_class(mod_ctx, view.notice, "music-hidden", libraryMessage.empty()), "Updating queue error");
        view.revision = libraryRevision;
        return MOD_OK;
    } catch (const std::exception& exception) { report(exception); return MOD_ERROR; }
}
ModResult build_library(ModContext*, UiWindowHandle, UiElementHandle left, UiElementHandle right, void*, ModError*) {
    try {
        libraryView = {};
        for (auto pane : {left, right}) check(svc_ui->elem_set_class(mod_ctx, pane, "music-library", true), "Styling library");
        ...
        check(svc_ui->pane_add_text(mod_ctx, left, "", &libraryView.queueSummary), "Building queue");
        const auto queueActions = add_row(queueHeader);
        check(add_control(queueActions, Control::AddFile, "Add file..."), "Building queue controls");
        check(add_control(queueActions, Control::AddFolder, "Add folder..."), "Building queue controls");
        check(add_control(queueActions, Control::ClearQueue, "Clear queue"), "Building queue controls");
        UiListDesc list = UI_LIST_DESC_INIT;
        list.on_pressed = list_pressed;
        check(svc_ui->pane_add_list(mod_ctx, left, &list, &libraryView.queue), "Building queue list");
        check(svc_ui->pane_add_text(mod_ctx, left, "", &libraryView.notice), "Building queue status");
        ...
        check(svc_ui->pane_add_text(mod_ctx, right, "", &libraryView.historySummary), "Building history");
        check(add_control(historyHeader, Control::ClearHistory, "Clear history"), "Building history controls");
        list.user_data = &libraryView;
        check(svc_ui->pane_add_list(mod_ctx, right, &list, &libraryView.history), "Building history list");
...
void open_window(ModContext*, void*) {
    ...
    desc.on_closed = [](ModContext*, UiWindowHandle, void*) { window = 0; windowView = {}; libraryView = {}; };
    try { check(svc_ui->window_push(mod_ctx, &desc, &window), "Opening music player"); }
    catch (const std::exception& exception) { report(exception); }
}
}
extern "C" {
MOD_EXPORT ModResult mod_initialize(ModError*) {
    UiStyleHandle style{};
    const auto styleResult = svc_ui->register_styles_file(mod_ctx, UI_SCOPE_WINDOW, "player.rcss", &style);
