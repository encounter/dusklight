...
void Decoder::open(std::span<const std::byte> bytes) {
    ...
    if (std::memcmp(bytes.data(), "OggS", 4) == 0) {
        ...
        if (!next->vorbis) {
            throw std::runtime_error{
                "Cannot decode this Ogg file. Choose Ogg Vorbis; Ogg Opus is not supported."};
        }
        ...
        if (!next->flac) {
            throw std::runtime_error{"Cannot decode this FLAC file."};
        }
        ...
        if (!drmp3_seek_to_pcm_frame(&next->mp3, 0)) {
            throw std::runtime_error{"Cannot rewind this MP3 file."};
        }
    } else {
        throw std::runtime_error{
            "Unsupported or damaged audio file. Choose MP3, Ogg Vorbis, FLAC, or WAV."};
    }
    ...
    if (!next->length) {
        throw std::runtime_error{"The file contains no decodable audio frames."};
    }
    impl = std::move(next);
}
...
uint32_t Decoder::read(std::span<float> interleaved) {
    ...
    const auto frames =
        std::min<size_t>(interleaved.size() / impl->channels, std::numeric_limits<int>::max() / 2);
    switch (impl->kind) {
    case Impl::Kind::Mp3:
        return drmp3_read_pcm_frames_f32(&impl->mp3, frames, interleaved.data());
    case Impl::Kind::Flac:
        return drflac_read_pcm_frames_f32(impl->flac, frames, interleaved.data());
    case Impl::Kind::Wav:
        return drwav_read_pcm_frames_f32(&impl->wav, frames, interleaved.data());
    case Impl::Kind::Vorbis:
        return stb_vorbis_get_samples_float_interleaved(impl->vorbis, impl->channels,
...
bool Decoder::seek(uint64_t frame) {
    if (!impl || frame >= impl->length) {
        return false;
    }
    switch (impl->kind) {
    case Impl::Kind::Mp3:
        return drmp3_seek_to_pcm_frame(&impl->mp3, frame);
    case Impl::Kind::Flac:
        return drflac_seek_to_pcm_frame(impl->flac, frame);
    case Impl::Kind::Wav:
        return drwav_seek_to_pcm_frame(&impl->wav, frame);
    case Impl::Kind::Vorbis:
        return frame <= std::numeric_limits<unsigned int>::max() &&
               stb_vorbis_seek(impl->vorbis, frame);
    default:
        return false;
    }
}

uint32_t Decoder::sample_rate() const {
    return impl ? impl->rate : 0;
}
...
uint64_t Decoder::length_frames() const {
